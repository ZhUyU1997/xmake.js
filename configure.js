...
function string_contains_star() {
    return string_contains("*")
}

function string_contains_star2() {
    return string_contains("**")
}
...
function string_to_array(s) {
    if (Array.isArray(s))
        return s
    return (s ?? "").trim().split(" ").filter(s => s !== "")
}
...
function _io_replace_file(infile, outfile, patterns) {
    let content = infile
...
function _get_target_basename(name) {
    let basename = _get_target_item(name, "basename");
    if (test_z(basename)) {
        basename = name;
    }
    return basename;
...
function add_defines(...args) {
    if (_loading_targets && test_z(_xmake_sh_option_current)) {
        let _public = false
        for (let define of args) {
            if (test_nq(define, "{public}"))
...
function add_udefines(...args) {
    if (_loading_targets && test_z(_xmake_sh_option_current)) {
        let _public = false
        for (let udefine of args) {
            if (test_nq(udefine, "{public}"))
        ...
        if (_public) {
            for (let udefine of args) {
                if (test_nq(udefine, "{public}"))
                    _add_target_item(_xmake_sh_target_current, "undefines_public", udefine);
            }
        }
    } else if (_loading_options && test_nz(_xmake_sh_option_current)) {
        for (let udefine of args) {
...
function _add_toolchain_toolset(name, kind, program) {
    const programs = _get_toolchain_item(name, `toolset_${kind}`)
    if (test_nz(programs))
...
function _vprint_configvar_value(name, value) {
    vprint(`  > replace ${name} -> ${value}`);
}

// vprint config variable in `${define name}`
function _vprint_configvar_define(name, value) {
    if (test_z(value)) {
...
const _replace_configvar_define = (name, value) => {
    let patterns
    if (test_z(value)) {
...
const _replace_configvar_value = (name, value) => {
    return [`\${${name}}`, value]
}
